...
/**
 * Clears Cloudinary proof screenshots older than 7 days from verification or task deadline,
...
export async function cleanupOnlyTaskScreenshots() {
  console.log('[Image Cleanup] Checking for task proof screenshots older than 7 days...');
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE ts.cloudinary_public_id IS NOT NULL
        ...
        AND (
          ts.verified_at < NOW() - INTERVAL '7 days'
          OR t.deadline < NOW() - INTERVAL '7 days'
        )
    `);
    ...
    console.log(`[Image Cleanup] Found ${result.rows.length} proof screenshot(s) older than 7 days to purge.`);
    ...
    for (const row of result.rows) {
      if (row.cloudinary_public_id) {
        ...
        await pool.query(`
          ...
          SET screenshot_url = 'PURGED_EXPIRED_7D',
